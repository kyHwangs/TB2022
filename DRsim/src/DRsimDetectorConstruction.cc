...
int DRsimDetectorConstruction::fNofModules = 2; // ! implement switch for 2 modules <-> 4 mudules
...
void DRsimDetectorConstruction::FiberImplement(G4int i, G4LogicalVolume* ModuleLogical__[], 
  ...
  for (int k = 0; k < NofPlate; k++) {
    for (int j = 0; j < NofFiber; j++) { 
      ...
      */
      G4float fX = -92.*mm/2 + k*1.5*mm + 1.*mm;
      G4float fY = -92.*mm/2 + j*1.5*mm + 1.*mm;
      fWhich = !fWhich;
  ...
  if ( doFiber ) {
    for (unsigned int j = 0; j<fFiberX.size(); j++) {
      ...
      if ( !fFiberWhich.at(j) ) { //c fibre
        ...
        tfiberUnitIntersection = new G4IntersectionSolid("fiberSQ",fiberUnit,module,0,G4ThreeVector(-fFiberX.at(j),-fFiberY.at(j),0.));
        fiberUnitIntersection__[i].push_back(new G4LogicalVolume(tfiberUnitIntersection,FindMaterial("G4_AIR"),name));
        new G4PVPlacement(0,G4ThreeVector(fFiberX.at(j),fFiberY.at(j),0),fiberUnitIntersection__[i].at(j),name,ModuleLogical__[i],false,j,checkOverlaps);
        ...
        new G4PVPlacement(0,G4ThreeVector((G4UniformRand()-0.5)*randDeviation,0.,0.),fiberCladIntersection__[i].at(j),name,fiberUnitIntersection__[i].at(j),false,j,checkOverlaps);
